...
class UsersService {
    ...
    async createUser(data) {
        try {
            ...
            userJson.connectedToSpotify = await user.isConnectedToSpotify();
            return {
                success: true,
                data: userJson,
            };
    ...
    async getUsers(filters = {}, options = {}) {
        try {
            ...
            const usersWithSpotifyStatus = await Promise.all(users.map(async (user) => {
                const userJson = user.toJSON();
                userJson.connectedToSpotify = await user.isConnectedToSpotify();
                return userJson;
            }));
